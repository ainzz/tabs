<template>
	<div :class="{ 'vue-bulma-tabs': true, [`is-layout-${layout}`]: true }">
		<div :class="{ tabs: true, [`is-${size}`]: size, [`is-${alignment}`]: alignment, [`is-${type}`]: type, 'is-fullwidth': isFullwidth }">
			...
			<tab-list>
				<li v-for="(tab, index) in tabPanes" role="tab" :class="{ 'is-active': isActived(index), 'is-disabled': tab.disabled, 'is-flex': true }" :aria-selected="isActived(index) ? 'true' : 'false'" :aria-expanded="isActived(index) ? 'true' : 'false'" :aria-disabled="tab.disabled ? 'true' : 'false'" :selected="isActived(index)" :disabled="tab.disabled" @click="select(index)">
					<a class="is-unselectable">
						<span :class="['icon', { 'is-small': size !== 'large'}, tab.color]" v-if="tab.icon">
							<i :class="tab.icon"></i>
						</span>
						<span>{{ tab.label }}</span>
					</a>
...
.vue-bulma-tabs {
	...
	&.is-layout-left {
		...
		.tab-list {
			flex-direction: column;
			align-items: flex-start; // max-width: calc(100% / 6);
			border-bottom: none;
	...
	&.is-layout-right {
		...
		.tab-list {
			...
			justify-content: flex-start; // max-width: calc(100% / 6);
			border-bottom: none;
